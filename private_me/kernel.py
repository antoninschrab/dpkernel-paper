...
class KGaussMult(Kernel):

    def __init__(self, sigma2):
        assert len(sigma2) == 2 
        assert sigma2[0] > 0.0 and sigma2[1] > 0.0, 'sigma2 must be > 0'
        self.sigma2_a = sigma2[0]
        self.sigma2_b = sigma2[1]
        self.kappa = 1.0

    def eval(self, X1, X2):
        """
        Evaluate the Gaussian kernel (Additive Average) on the two 2d numpy arrays.
        ...
        K = 0.5 * np.exp(-D2/self.sigma2_a) + 0.5 * np.exp(-D2/self.sigma2_b)
        return K
    ...
    def pair_eval(self, X, Y):
        ...
        D2 = np.sum( (X-Y)**2, 1)
        Kvec = 0.5 * np.exp(-D2/self.sigma2_a) + 0.5 * np.exp(-D2/self.sigma2_b)
        return Kvec

    def __str__(self):
        return "KGauss(w2a={}, w2b={})".format(self.sigma2_a, self.sigma2_b)
